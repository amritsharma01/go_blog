...
func (r *postRepository) Create(post *models.Post) error {
	if err := r.db.Create(post).Error; err != nil {
		return errors.Internal("Failed to create post", err)
	}
	return nil
}
...
func (r *postRepository) FindByID(id uint) (*models.Post, error) {
	...
	if err := r.db.Preload("Author").Preload("Category").First(&post, id).Error; err != nil {
		...
		return nil, errors.Internal("Failed to fetch post by ID", err)
	}
	return &post, nil
}
...
func (r *postRepository) FindAll(search, categoryID, authorID string, offset, limit int) ([]models.Post, int64, error) {
	...
	if err := query.Count(&count).Error; err != nil {
		return nil, 0, errors.Internal("Failed to count posts", err)
	}

	if err := query.Order("created_at DESC").Limit(limit).Offset(offset).Find(&posts).Error; err != nil {
		return nil, 0, errors.Internal("Failed to fetch posts", err)
	}
...
// 	if err := query.Count(&count).Error; err != nil {
// 		return nil, 0, errors.Internal("Failed to count posts by author", err)
// 	}

// 	if err := query.Limit(limit).Offset(offset).Find(&posts).Error; err != nil {
// 		return nil, 0, errors.Internal("Failed to fetch posts by author", err)
// 	}
...
func (r *postRepository) Update(post *models.Post) error {
	if err := r.db.Save(post).Error; err != nil {
		return errors.Internal("Failed to update post", err)
	}
	return nil
}
...
func (r *postRepository) Delete(post *models.Post) error {
	if err := r.db.Delete(post).Error; err != nil {
		return errors.Internal("Failed to delete post", err)
	}
	return nil
}
...
func (r *postRepository) FindDuplicate(title string, authorID uint) (*models.Post, error) {
	...
	if err := r.db.Where("title = ? AND author_id = ?", title, authorID).First(&post).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return nil, nil // No duplicate found is not an error
		}
		return nil, errors.Internal("Failed to check for duplicate post", err)
	}
	return &post, nil
}
